        ...
        <div className="flex gap-8">
          ...
          <aside className="hidden lg:block w-64 flex-shrink-0">
            <div className="sticky top-24">
              <div className="toc">
                <h4 className="text-sm font-bold text-green-300 uppercase tracking-wider mb-4">📑 On This Page</h4>
                <nav>
          ...
          <article className="flex-1 min-w-0">
            ...
            {related.length > 0 && (
              <section className="mt-16 pt-10 border-t border-green-900/30">
                <h2 className="text-2xl font-bold text-green-200 mb-6">📚 Related Guides</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {related.map(r => (
                    <a key={r.slug} href={'/articles/' + r.slug} className="related-card group">
                      <span className="tag text-xs">{r.category}</span>
                      <h3 className="font-semibold text-gray-200 mt-2 mb-1 group-hover:text-green-300 transition-colors line-clamp-2">{r.title}</h3>
            ...
            {(() => {
              ...
              return (
                <section className="mt-12 pt-8 border-t border-green-900/30">
                  <h2 className="text-xl font-bold text-green-200 mb-4">🔗 See Also</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {seeAlso.map((a: any) => (
                      <a key={a.slug} href={'/articles/' + a.slug} className="text-sm text-green-400 hover:text-green-200 transition-colors">{a.title}</a>
                    ))}
                  </div>
                </section>
              );
